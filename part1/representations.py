...
def get_features(
    ...
    for features, labels in dataloader:
        with torch.no_grad():
            ...
            new_features = feature_extractor(features)
            new_features = new_features.to(device)
            features_list.append(new_features.cpu().numpy()) # Necessary for the CLIP architecture
            labels_list.append(labels.cpu().numpy())
        
    return np.concatenate(features_list), np.concatenate(labels_list), dataset.num_classes
...
def train_linear_probe(
    ...
    dataloader = DataLoader(features_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True)

    
    linear_probe = nn.Linear(features_dataset.features.shape[1], features_dataset.num_classes)
    ...
    loss_fn = torch.nn.CrossEntropyLoss() #for the outputs of the linear probes
    ...
    for _ in range(num_epochs):
        ...
        for features, labels in dataloader:
            ...
            optimizer.zero_grad()
            new_labels = linear_probe(features) #going to train a simple
            new_loss = loss_fn(new_labels, labels)

            current_losses.append(new_loss.item())
...
def find_nearest_neighbors(
    ...
    for x in range(features.shape[0]):
        result = sklearn.metrics.pairwise.cosine_similarity([features[x]], [query_features])
        cosine_similarities.append(result[0][0])

    cosine_similarities = np.array(cosine_similarities)

    indices = np.argsort(cosine_similarities)

    final_indices = indices[::-1][:k] #getting the first k

    similarities = [cosine_similarities[v] for v in final_indices]


    return final_indices, similarities
